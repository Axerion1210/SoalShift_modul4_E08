		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	close(fd);
	res = mkdir("/Backup", 0775);
	if (res == -1)
		return -errno;

	////////
		char depan[1000], belakang[10], timestamp[30], tujuan[2000];
		char *ptr;
		int index;
		ptr = strchr(sumber, '.');
		if(ptr){
		...
		strncpy(tujuan,sumber,index);
		ext = strrchr(sumber,'.');
	/////	
	//sudah	//cari posisi titik (.)terakhir strrchr()
	//sudah	//ambil string smpe . terakhir strncpy(depan,namafile,posisi)
	//sudah	//ambil string mulai dari . terakhir strncpy(belakang,namafile+posisi,strlen-posisi)
		//ambil timestamp
		//sprintf(namafile,"/Backup/%s%s%s",depan,timestamp,belakang)
		//encrypt
		//hasilnya ditambahkan setelah fpath
